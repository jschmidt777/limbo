...
#Shows the reocrd for an item selected
...
#Shows the admin information for the admin that was selected to confirm their deletion
...
#Shows link records for the items
...
#Displays link for admin users to choose which one to delete
...
#Loads the new item page, I should probably put this into limbo login tools but it does just load the page so maybe.
...
function insert_lost_record($dbc, $description, $location_name, $owner, $room) {
  $query = 'INSERT INTO stuff(description, location_name, owner, room, create_date, status) 
				   VALUES ("' . $description. '" , "' . $location_name . '", "' . $owner . '", "' . $room . '" , NOW(), "lost")' ;
  show_query($query);
